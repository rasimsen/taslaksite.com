...
if(isset($_REQUEST["Submit"])){
	...
	if(strlen($ad)==0) $procesOK.='<h2><font color="red">Lütfen Adýnýzý girin</font></h2>';	
	if(strlen($konu)==0) $procesOK.='<h2><font color="red">Lütfen mesajýnýz konusunu girin</font></h2>';	
	if(strpos($email,"@")===false || strpos($email,".")===false) $procesOK .='<h2><font color="red">Lütfen email adresinizi kontrol ediniz</font></h2>';	
	...
	if(strlen($processOK)==0){
		...
		if($sen->veriEkle($db,tILETISIM,$mesaj_veri,0)) 
			$sen->mesajBasarili("Mesajýnýz Ýletilmiþtir","Baþarýlý..");
