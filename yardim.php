 ...
 * yardým sol menü
 */
$yardim=new Yardim($db);
$sen->smarty->assign('yardim_sol_menu',$yardim->yardimSolMenu($db));
//pre($yardim->yardimSolMenu($db));

$arama=new Arama($db);
$arama_text=trim($sen->alRequest("arama_text"));
$arama_turu=trim($sen->alRequest("arama_turu"));
$sayfa='YARDIM';//trim($sen->alRequest("sayfa"));
