...
class upload {
	...
	function fileSizeControl($size){
		if (!$size >$this->maxfile) {
			$this->error[]="Fotoðraf boyutu en fazla ".($this->maxfile/(1024*1024))." mb olabilir";
			return false;
		}else
			return true;
			...
			if($photo['name'][$i]=="") continue;
				...
				for($j=0;$j<$tip;$j++){
					...
					if(($thumb=="T" && $tip==1) || ($tip==2 && $j==0)){
							...
							$uploadedPhotos[$i][THUMB_WIDTH]=$ns["width"];
							$uploadedPhotos[$i][THUMB_HEIGHT]=$ns["height"];
					}else{	
							$file=$prod_img_new;
							$ns=$this->resizeImage($aspect_ratio,$sizes,array("width"=>$this->newwidth,"height"=>$this->newheight));							
							$uploadedPhotos[$i][WIDTH]=$ns["width"];
							$uploadedPhotos[$i][HEIGHT]=$ns["height"];
					}	
  ...
  function file_upload($dosya,$save_path,$prefix){
	 ...
	 */
  	if($prefix=="CV_" && !$this->izinVerilenDosyami($dosya["name"])){
